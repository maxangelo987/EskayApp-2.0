...
letters = ["W","Z","1","2","3","4","5","6","7","8","9","0","ᜀ","ᜊ","ᜃ","ᜇ","ᜁ","ᜑ","ᜄ","ᜎ","ᜋ","ᜈ"]
#letters = ["V","E","Y","M","K"]
letters_1 = ["O","M"]

for mir in letters_1:
    for ch in letters:
        y = 'TRAIN_' + ch + mir +'.jpg'

        #imgTrainingNumbers = cv2.imread("TRAIN_A.jpg")  # read in training numbers image
        #imgTrainingNumbers = cv2.imread(y)  # read in training numbers image
        imgTrainingNumbers = cv2.imdecode(np.fromfile(y, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
        ...
        intValidChars = [ord('W'), ord('Z'), ord('1'), ord('2'), ord('3'), ord('4'), ord('5'), ord('6'), ord('7'),
                         ord('8'), ord('9'), ord('0'), ord('ᜀ'), ord('ᜊ'), ord('ᜃ'), ord('ᜇ'), ord('ᜁ'), ord('ᜄ'),
                         ord('ᜑ'), ord('ᜎ'), ord('ᜋ'), ord('ᜈ')]
        ...
        print("\n\nᜑᜓᜋᜈ᜔ ᜈ ᜀᜅ᜔ ᜉᜄ᜔ᜆᜓᜂᜈ᜔᜶ Training complete!\n")

        y_c = 'CLASSIFICATIONS_' + ch + mir+'.txt'
        y_i = 'FLATTENED_IMAGES_' + ch + mir +'.txt'
        #y_c = 'CLASSIFICATIONS.txt'
        #y_i = 'FLATTENED_IMAGES.txt'
        np.savetxt(y_c, npaClassifications)  # write flattened images to file
        np.savetxt(y_i, npaFlattenedImages)  #
